...
| D-6 | **[БЛОКЕР 152-ФЗ] grade=8 может означать возраст <14 лет.** По 152-ФЗ ст.9 ч.1 согласие на обработку ПД для лиц до 14 лет даёт родитель/опекун, а не сам ребёнок. Продуктовое решение фаундера: (а) ограничить регистрацию grade≥9, либо (б) добавить верификацию возраста (например, подтверждение от родителя через ссылку). | Полноценная реализация grade=8 откладывается до закрытия этой развилки. До решения: регистрация grade=8 допускается с предупреждением (текущий инвариант сохранён), но правовой статус согласия не проверяется системой и остаётся на ответственности фаундера. |
...
| **DailySession** | Агрегат активности за учебный день | `user_id`, `date`, `lessons_completed`, `reviews_completed`, `morning_warmup_done`, `missed_day_end` (bool, флаг: evt_day_end наступил, но ученик был не в состоянии registered — streak_update отложен до следующего evt_open_app) | N:1 → User |
...
- При регистрации ученика с `grade=8` — система показывает предупреждение «Курс рассчитан на учеников 9–11 классов; для 8 класса часть тем может быть рановата» и требует явного подтверждения для продолжения. Регистрация не блокируется, но предупреждение обязательно. Полноценная реализация откладывается до решения D-6 (152-ФЗ).
...
transitions:
  ...
  - from: onboarding
    ...
    to: registered
    guard: "name присутствует AND grade в диапазоне 8..11 AND pd_consent=true; при grade=8 — предупреждение подтверждено (D-6 открыта)"
...
# evt_delete_account в состояниях урока (lesson_*) и repeat_*/review_queue/daily_*: невозможно в v1 — удаление доступно только из registered, onboarding и course_complete; при длительном нахождении в уроке — пользователь должен завершить/выйти в registered
...
> Охват v3: только роль «ученик». Сценарии с другими ролями — в будущих спеках.
...
| 2 | Система | Показывает экран регистрации: поля «Как тебя зовут?» (никнейм) и «В каком ты классе?» (выбор 8–11); показывает ссылку на Политику обработки персональных данных; показывает чекбокс «Я даю согласие на обработку ПД» (обязателен) | Экран регистрации отображён |
| 2а | Система | Если ученик выбирает класс «8» — отображает предупреждение: «Курс рассчитан на 9–11 классы; для 8 класса часть тем может быть рановата. Продолжить?» (D-6 открыта) | Предупреждение показано, кнопка подтверждения |
...
| 6 | Система | Создаёт User (role=student, name=Иван, grade=9, pd_consent_at=now, pd_consent_version=«v1»), StudentProfile, Streak (current=0), DailySession (missed_day_end=false), Session (httpOnly cookie, 30 дней) | БД обновлена |
...
| EC-18 | grade=8 при регистрации | Система показывает предупреждение «Курс рассчитан на 9–11 классы»; требует явного подтверждения; регистрация не блокируется после подтверждения. Полноценная обработка отложена до решения D-6 (152-ФЗ) |
...
│   │   ├── App.tsx
│   │   ├── pages/
│   │   │   ├── Onboarding.tsx   # Регистрация (никнейм + класс + чекбокс ПД + предупреждение grade=8)
...
│   └── student_lesson_fsm_v3.md  # Этот файл
├── docker-compose.yml
...
*Спека v3 закрывает все блокеры и существенные замечания по ревью А4 v2. Открытая развилка D-6 (152-ФЗ, grade=8) зафиксирована явно и ждёт решения фаундера. Роли родитель/учитель/репетитор, мессенджер-адаптеры — отдельные спеки после валидации ученической поверхности.*
