...
|----------|----------|-------------------|-------|
| **User** | Аккаунт пользователя любой роли | `id`, `role` (student/parent/teacher/tutor), `name` (никнейм/отображаемое имя; настоящее ФИО не собирается и не запрашивается), `grade` (только student), `created_at`, `pwa_push_token`, `pd_consent_at` (datetime — момент принятия политики ПД), `pd_consent_version` (строка версии политики) | 1:1 → StudentProfile (если role=student) |
| **StudentProfile** | Расширенный профиль ученика | `user_id`, `current_lesson_id`, `course_started_at`, `last_active_at` (datetime, для контроля 90-дневного периода неактивности), `fsm_state` (string, snake_case — каноничное текущее состояние FSM; обновляется атомарно с Progress при каждом FSM-переходе; при расхождении Progress.status и fsm_state — fsm_state приоритетен) | 1:1 → User; 1:N → Progress; 1:1 → Streak; 1:N → ReviewQueue |
...
| **DailySession** | Агрегат активности за учебный день | `user_id`, `date`, `lessons_completed`, `reviews_completed`, `morning_warmup_done`, `missed_day_end` (bool, флаг: evt_day_end наступил, но ученик был не в состоянии registered — streak_update отложен до следующего evt_open_app из registered) | N:1 → User |
...
| **Links (StudentLink)** | Связь ученика со взрослым пользователем (родитель/учитель/репетитор) | `id`, `student_user_id`, `adult_user_id`, `link_type` (parent/teacher/tutor), `link_code` (короткий код для привязки), `created_at`, `is_active` (bool) | N:1 → User (student); N:1 → User (adult) |
| **Classes** | Класс/группа учеников | *вне охвата данной спеки; deny для роли student по всем операциям* | — |

### Инварианты модели данных

- Регистрация собирает только: **никнейм (отображаемое имя)** + **класс** + **согласие на политику ПД** (152-ФЗ, минимизация ПД). Никакого телефона/email. Настоящее ФИО не собирается и не запрашивается.
...
- **В production-среде до закрытия D-6 фаундером guard регистрации ограничен: grade IN (9, 10, 11). grade=8 доступно только в test/staging среде.** При grade=8 в staging — система показывает предупреждение «Курс рассчитан на учеников 9–11 классов; для 8 класса часть тем может быть рановата» и требует явного подтверждения.
- Аккаунт неактивен, если `StudentProfile.last_active_at < now - 90 дней` (см. F-11).
- `DailySession.missed_day_end=true` означает, что scheduler выполнил день, но ученик находился не в состоянии `registered`; streak_update будет выполнен при следующем `evt_open_app` из `registered` (см. §2 — подход к evt_day_end).
- `StudentProfile.fsm_state` — единственный канонический источник текущего состояния FSM. При расхождении с `Progress.status` — `fsm_state` приоритетен. Обновляется атомарно с Progress в одной БД-транзакции.

---

## 2. Конечный автомат (FSM) — роль «Ученик»

### Подход к evt_day_end (обоснование)

evt_day_end — системный broadcast-event, генерируемый scheduler в 23:59. Прямые переходы из всех состояний (кроме `registered`) технически возможны, но создают избыточную сложность: 20+ дополнительных переходов, большинство из которых ведут в одно и то же состояние `streak_update`. Выбран **альтернативный подход**: scheduler при запуске evt_day_end проверяет текущее состояние ученика. Если ученик в состоянии `registered` — немедленно выполняет streak_update. Если ученик в любом другом состоянии — устанавливает `DailySession.missed_day_end=true` и откладывает streak_update. При следующем `evt_open_app` из `registered` — если `missed_day_end=true`, fsm_service немедленно генерирует `evt_day_end` перед переходом в `daily_start`. Это консистентно с логикой FSM (переходы только из определённых состояний) и не допускает потери streak-данных.

Особые случаи:
- `daily_done + evt_day_end`: ученик завершил день — streak уже обновлён через `evt_session_end → registered`; повторный `evt_day_end` из `registered` — идемпотентен (проверка `last_active_date == today`).
- `daily_blocked + evt_day_end`: failed_today зафиксирован; streak не должен сбрасываться, если `reviews_completed >= 1` (разминка была завершена). Логика streak_update учитывает это.
- `repeat_evening_active + evt_day_end` (23:59): если R2 не завершён, `missed_day_end=true`; при оценке streak засчитывается как `reviews_completed=1` только если R2 фактически был завершён до 23:59.
...
stateDiagram-v2
    ...
    registered --> daily_start : evt_open_app [missed_day_end=false]
    registered --> streak_update : evt_open_app [missed_day_end=true — отложенный streak_update]
    ...
    %% evt_day_end: scheduler sets DailySession.missed_day_end=true; переход не генерируется напрямую — обрабатывается при следующем evt_open_app из registered
    ...
    lesson_hook --> registered : evt_cancel_lesson [прогресс сохранён; урок возобновляем]
    lesson_theory --> lesson_example : evt_theory_read
    lesson_theory --> registered : evt_cancel_lesson [прогресс сохранён; урок возобновляем]
    lesson_example --> lesson_training : evt_example_read
    lesson_example --> registered : evt_cancel_lesson [прогресс сохранён; урок возобновляем]

    lesson_training --> lesson_training : evt_answer_correct [остались тренировочные вопросы]
    lesson_training --> lesson_main_question : evt_answer_correct [все Q1..Q3 пройдены — движок автоматически переходит в lesson_main_question без дополнительного события]
    ...
    lesson_training --> registered : evt_cancel_lesson [прогресс сохранён; урок возобновляем]
    ...
    lesson_main_question --> registered : evt_cancel_lesson [прогресс сохранён; урок возобновляем]
    lesson_theory_review --> lesson_main_question_backup : evt_theory_reviewed
    lesson_theory_review --> registered : evt_cancel_lesson [прогресс сохранён; урок возобновляем]
    ...
    lesson_main_question_backup --> registered : evt_cancel_lesson [прогресс сохранён; урок возобновляем]
    ...
    lesson_final --> registered : evt_cancel_lesson [прогресс сохранён; урок возобновляем]

    lesson_failed --> daily_blocked : evt_lesson_fail_confirmed
    lesson_failed --> registered : evt_cancel_lesson [прогресс сохранён: status=failed_today; урок добавлен в review_queue]
    ...
    repeat_evening_active --> repeat_evening_active : evt_open_app [самопетля: показать «Вечернее повторение ждёт тебя»]
    ...
    daily_done --> registered : evt_next_day [следующий учебный день]
    daily_blocked --> registered : evt_next_day [следующий учебный день]
    daily_blocked --> daily_blocked : evt_open_app [самопетля: показать «Урок доступен завтра»]

    streak_update --> registered : evt_streak_updated
    %% все события кроме evt_streak_updated невозможны в streak_update — переход выполняется синхронно, пользователь не взаимодействует

    course_complete --> registered : evt_open_app [для доступа к настройкам и удалению аккаунта]
    course_complete --> unregistered : evt_delete_account [каскадное удаление]
```
...
states:
  ...
  - id: course_complete
    type: normal
    # course_complete — НЕ end: позволяет evt_open_app → registered (настройки/удаление) и evt_delete_account → unregistered
...
transitions:
  ...
  - from: registered
    event: evt_delete_account
    to: unregistered
    guard: "удалить User, StudentProfile, Progress, Streak, ReviewQueue, ReminderState, DailySession, Session, Links; инвалидировать push_token; ответить 204"

  # --- Вход в приложение ---
  - from: registered
    event: evt_open_app
    to: daily_start
    guard: "обновить StudentProfile.last_active_at = now; DailySession.missed_day_end=false"

  - from: registered
    event: evt_open_app
    to: streak_update
    guard: "DailySession.missed_day_end=true — выполнить отложенный streak_update перед daily_start; после evt_streak_updated перейдёт в registered, затем evt_open_app повторно"

  # --- Streak (evt_day_end из registered) ---
  - from: registered
    event: evt_day_end
    to: streak_update
    guard: "ежедневный job 23:59; при date.weekday()==0 (понедельник) — сбросить Streak.freeze_used_this_week=false; для всех остальных состояний (не registered) — scheduler выставляет DailySession.missed_day_end=true, streak_update отложен"

  - from: streak_update
    event: evt_streak_updated
    to: registered
    guard: "если DailySession.lessons_completed >= 1 ИЛИ reviews_completed >= 1: streak+1; иначе: применить freeze (если freeze_used_this_week=false) или сбросить streak в 0; при daily_blocked: streak не сбрасывается если reviews_completed >= 1"
  # все события кроме evt_streak_updated невозможны в streak_update — переход выполняется синхронно, пользователь не взаимодействует
  ...
  - from: lesson_hook
    ...
    to: registered
    guard: "сохранить Progress.current_message_id и Progress.status=in_progress; StudentProfile.fsm_state=registered; урок можно возобновить"
  ...
  - from: lesson_training
    ...
    guard: "все тренировочные вопросы (Q1..Q3) пройдены; движок после evt_answer_correct (last) автоматически переходит в lesson_main_question без дополнительного события"
  ...
  - from: lesson_failed
    event: evt_cancel_lesson
    to: registered
    guard: "прогресс сохраняется: Progress.status=failed_today (уже зафиксирован или фиксируется сейчас); добавить в review_queue (reason=interval_1d, due_date=tomorrow); StudentProfile.fsm_state=registered"
  ...
  - from: repeat_evening_active
    event: evt_open_app
    to: repeat_evening_active
    guard: "самопетля: показать «Вечернее повторение ждёт тебя»"
  ...
  - from: daily_blocked
    event: evt_next_day
    to: registered
    guard: "наступил следующий учебный день (scheduler); side-effect: Progress.status=failed_today → not_started для урока, заблокированного сегодня"

  - from: daily_blocked
    event: evt_open_app
    to: daily_blocked
    guard: "самопетля: показать «Урок доступен завтра»"

  # --- course_complete ---
  - from: course_complete
    event: evt_open_app
    to: registered
    guard: "для доступа к настройкам, статистике и управлению аккаунтом (включая удаление)"

  - from: course_complete
    event: evt_delete_account
    to: unregistered
    guard: "каскадное удаление всех ПД: User, StudentProfile, Progress, Streak, ReviewQueue, ReminderState, DailySession, Session, Links; инвалидация push_token; ответить 204"
...
# evt_training_complete удалён из спеки v4: движок после evt_answer_correct (last) автоматически переходит в lesson_main_question без дополнительного события; evt_training_complete как самостоятельное событие более не существует
# evt_cancel_lesson в состояниях repeat_*/review_queue/daily_*/streak_update/course_complete: невозможно — нет активного урока для отмены
# evt_day_end из состояний, отличных от registered: scheduler выставляет DailySession.missed_day_end=true; streak_update откладывается до следующего evt_open_app из registered
# все события кроме evt_streak_updated невозможны в streak_update — переход выполняется синхронно, пользователь не взаимодействует
```
...
| student | user_account_own | read | false | — |
| student | user_account_own | write | false | — |
| student | user_account_own | create | false | — |
| student | user_account_own | delete | true | DELETE /api/account — основной эндпоинт каскадного удаления всех ПД (152-ФЗ); user_profile_own/delete — синоним, перенаправляет на тот же эндпоинт |
...
| student | classes | read | false | вне охвата данной спеки; deny для роли student |
...
permissions:
  ...
  - role: student
    ...
    operation: delete
    allow: true
    guard: "каскадное удаление всех ПД через evt_delete_account (152-ФЗ); синоним user_account_own/delete — перенаправляет на DELETE /api/account"
  ...
  # --- user_account_own ---
  # DELETE /api/account — основной эндпоинт каскадного удаления (152-ФЗ)
  # user_profile_own/delete — синоним, перенаправляет на тот же эндпоинт
  - role: student
    resource: user_account_own
    operation: delete
    allow: true
    guard: "каскадное удаление: User, StudentProfile, Progress, Streak, ReviewQueue, ReminderState, DailySession, Session, Links; инвалидация push_token; ответить 204"
  ...
  # --- classes (вне охвата данной спеки) ---
  - role: student
    resource: classes
    operation: read
    allow: false
    guard: "вне охвата данной спеки; deny для роли student"
...
> Охват v4: только роль «ученик». Сценарии с другими ролями — в будущих спеках.
...
| 6 | Система | Создаёт User (role=student, name=Иван, grade=9, pd_consent_at=now, pd_consent_version=«v1»), StudentProfile (fsm_state=registered), Streak (current=0), DailySession (missed_day_end=false), Session (httpOnly cookie, 30 дней) | БД обновлена |
| 7 | Система | Переводит в `registered`, показывает первый урок | Состояние `registered` → `daily_start` → `lesson_select` |

**Постусловие:** User и StudentProfile созданы; ПД — только никнейм и класс; согласие зафиксировано с timestamp. Настоящее ФИО не собрано.
...
| Шаг | Актор | Действие | Результат |
|-----|-------|----------|-----------|
| 1 | Ученик | Открывает PWA | `evt_open_app` → проверка missed_day_end=false → `daily_start`; обновляется `last_active_at` |
...
| 11 | Ученик | Отвечает верно на Q1, Q2, Q3 | `evt_answer_correct` × 3 → после Q3 (последний верный ответ): движок автоматически переходит в `lesson_main_question` без дополнительного события |
...
| 15 | Система | Проверяет: не все 27 уроков passed → `evt_lesson_complete` → `repeat_1h_pending`; планирует R1 (через 1 ч), R2 (~21:00); добавляет в review_queue interval_1d/3d/7d/14d/30d (passed_attempt_1 — запись passed_attempt_2 НЕ добавляется); записывает DailySession | Урок полностью засчитан |
...
| 17 | Система | Показывает финал, Progress.status=passed, passed_on_attempt=2; при evt_lesson_complete: passed_attempt_2 — interval-записи уже добавлены в review_queue при evt_main_correct_attempt2; evt_lesson_complete только подтверждает статус passed и запускает R1/R2; дублирования нет | Урок засчитан |
...
| 5 | Scheduler | Следующий учебный день | `evt_next_day` → `registered`; side-effect: Progress.status=failed_today → not_started |
...
| 2 | Ученик | Открывает PWA | `evt_open_app` → проверка missed_day_end → `daily_start` |
...
| Шаг | Актор | Действие | Результат |
|-----|-------|----------|-----------|
| 1 | Scheduler | 23:59 — ученик в состоянии `registered` → `evt_day_end` → `streak_update`; если понедельник — сбросить freeze_used_this_week=false | — |
...
| 3 | Система | Каскадно удаляет по DELETE /api/account: User, StudentProfile, Progress, Streak, ReviewQueue, ReminderState, DailySession, Session, Links; инвалидирует push_token | Все ПД удалены |
...
### Сценарий S-09: Удаление аккаунта из course_complete

**Участники:** ученик  
**Предусловие:** ученик завершил все 27 уроков, находится в состоянии `course_complete`

| Шаг | Актор | Действие | Результат |
|-----|-------|----------|-----------|
| 1 | Ученик | Открывает PWA | `evt_open_app` → `registered` (для доступа к настройкам) |
| 2 | Ученик | Переходит в настройки → «Удалить аккаунт» | `evt_delete_account` → `unregistered` |
| 3 | Система | Каскадное удаление по DELETE /api/account | Все ПД удалены |

**Альтернатива:** из `course_complete` напрямую `evt_delete_account` → `unregistered` (оба пути поддержаны FSM).

---

### Сценарий S-10: Выход из урока без завершения (evt_cancel_lesson)

**Участники:** ученик  
**Предусловие:** ученик находится в `lesson_training` (или любом другом lesson_* состоянии)

| Шаг | Актор | Действие | Результат |
|-----|-------|----------|-----------|
| 1 | Ученик | Нажимает кнопку «Выйти из урока» | `evt_cancel_lesson` |
| 2 | Система | Сохраняет Progress.current_message_id и Progress.status=in_progress; обновляет StudentProfile.fsm_state=registered | Progress не потерян |
| 3 | Система | Переходит в `registered` | Ученик на главном экране |
| 4 | Ученик | При следующем входе нажимает «Продолжить урок» | Урок возобновляется с сохранённой позиции |

**Постусловие:** Progress.status=in_progress; урок возобновляем; evt_delete_account доступен из registered.
...
| EC-02 | Ученик закрывает браузер в середине урока (между Q1 и Q2) | Progress.current_message_id сохранён при каждом FSM-переходе; StudentProfile.fsm_state сохранён; при следующем входе урок продолжается с сохранённой позиции |
...
| EC-22 | Ученик в daily_blocked открывает PWA | Самопетля daily_blocked + evt_open_app → daily_blocked; показывается сообщение «Урок доступен завтра»; новые уроки не запускаются |
| EC-23 | Краш браузера в середине FSM-перехода (bfcache + race) | При следующем запросе fsm_service читает StudentProfile.fsm_state из БД как каноничное текущее состояние; не доверяет клиентскому параметру; расхождение Progress.status и fsm_state разрешается в пользу fsm_state |
...
| F-05 | Scheduler (day_end / evening_time) не запустился | Streak не обновился, R2 не отправлен; при следующем запуске scheduler — проверяет пропущенные даты и применяет с флагом «запоздало»; missed_day_end=true обрабатывается при следующем evt_open_app | Да — при следующем запуске scheduler |
...
│   │   ├── student.py           # FSM-эндпоинты: start_lesson, answer, next_message, cancel_lesson
│   │   ├── progress.py          # GET progress, streak, review_queue (read-only для клиента)
│   │   └── push.py              # Регистрация push_token
│   ├── services/
│   │   ├── fsm_service.py       # Бизнес-логика переходов FSM (mastery learning rules); читает fsm_state из StudentProfile как текущее состояние — не доверяет клиенту
│   │   ├── streak_service.py    # Streak update, freeze logic, weekly reset, missed_day_end
│   │   ├── review_service.py    # review_queue: добавление, выборка, обновление интервалов
│   │   ├── account_service.py   # Удаление аккаунта (DELETE /api/account), cleanup неактивных
│   │   └── reminder_service.py  # Триггеры напоминаний
│   ├── auth/
│   │   └── deps.py              # Зависимости: get_current_user, check_student_owns_resource
│   └── tests/
│       ├── test_fsm.py          # FSM-переходы: happy path + edge cases + evt_cancel_lesson
│       ├── test_mastery.py      # Логика 3 попыток, review_queue
│       ├── test_streak.py       # Streak update, freeze, missed_day_end
...
│   │   ├── App.tsx
│   │   ├── pages/
│   │   │   ├── Onboarding.tsx   # Регистрация (никнейм + класс + чекбокс ПД + предупреждение grade=8 в staging)
│   │   │   ├── DailyStart.tsx   # Главный экран дня
│   │   │   ├── MorningWarmup.tsx
│   │   │   ├── LessonPlayer.tsx # Рендер сообщений урока, кнопки ответов, кнопка «Выйти из урока»
...
│   ├── student_lesson_fsm_v2.md
│   ├── student_lesson_fsm_v3.md
│   └── student_lesson_fsm_v4.md  # Этот файл
├── docker-compose.yml
...
| routers → fsm_service | При получении запроса fsm_service читает `StudentProfile.fsm_state` из БД как текущее состояние FSM — не доверяет клиентскому параметру. `fsm_state` обновляется атомарно с Progress в одной транзакции при каждом переходе. |
...
| scheduler → fsm_service | Scheduler вызывает fsm_service с событиями `evt_1h_elapsed`, `evt_evening_time`, `evt_day_end` через внутренний API (не через HTTP); при day_end в понедельник — передаёт флаг reset_freeze=True; если ученик не в состоянии registered — выставляет DailySession.missed_day_end=true вместо прямого вызова streak_update |
...
| account_service → DELETE /api/account | Основной эндпоинт каскадного удаления. user_profile_own/delete — синоним, перенаправляет на тот же эндпоинт. Каскад: User, StudentProfile, Progress, Streak, ReviewQueue, ReminderState, DailySession, Session, Links; инвалидация push_token; ответ 204 |
| fsm_service → lesson_engine | После evt_answer_correct: движок проверяет, остались ли тренировочные вопросы. Если все Q1..Q3 пройдены — движок автоматически генерирует переход в lesson_main_question без дополнительного события от клиента. evt_training_complete как самостоятельное событие упразднён в v4. |
