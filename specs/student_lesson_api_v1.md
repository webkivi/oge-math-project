...
> **РЕВИЗИЯ R3 (Вариант А, hook-маппинг) (2026-06-23, решение фаундера, формализуется без validator — чисто транспортное уточнение; ДОУТОЧНЕНА по ревью Критика «GO WITH REVISIONS», блокеров нет — 3 точечные правки §3.1-R3/§4.5-R3/§9: пометка встречной правки A6 §3.2 на приёмку, полнота таблицы `fsm_state→step` при `total=6`, ссылка на `keeper.REQUIRED_STAGES` для отсечения мнимой дыры «hook без theory»):** при кодировании п.1a подтверждён факт: НИ В ОДНОМ уроке Блока 1 (все 9 уроков) НЕТ стадии `hook` — уроки начинаются со стадии `theory`; `keeper.LESSON_STAGES` НЕ содержит `hook`. Но FSM v4 / `lesson_engine` имеют состояние `lesson_hook` (`lesson_select --evt_start_lesson--> lesson_hook --evt_hook_read--> lesson_theory`), а §1.3/§4.5 считали hook шагом 1 из 6 — рендерить там нечего. По Варианту А внесены 3 ТРАНСПОРТНЫЕ правки БЕЗ изменения FSM/движка/keeper: (1) §3.1 — правило hook-маппинга: если у урока в CSV НЕТ сообщения стадии `hook` (текущая реальность всего контента), `fsm_service` при старте урока (`evt_start_lesson → lesson_hook`) АВТОМАТИЧЕСКИ деривит `evt_hook_read` на том же шаге, ученик попадает СРАЗУ в `lesson_theory` на `theory[0]` (`lesson_hook` — нулевая-контентная пройная-стадия, без отдельного клиентского «Дальше»; по аналогии с серверной деривацией `evt_warmup_complete`/`evt_session_end`); если hook-сообщение в будущем ПОЯВИТСЯ — рендерится штатно, `evt_hook_read` деривится по «Дальше». (2) §4.5/§4.1 — ремап `LessonProgress`: при отсутствии hook прогресс из 5 шагов (`1=theory … 5=final`, `total=5`), при наличии hook — 6 шагов как было; для текущего контента Блока 1 прогресс `*/5`, старт `theory=1/5`. (3) §1.3 — кросс-ссылка: `evt_hook_read` может деривиться сервером АВТОМАТИЧЕСКИ на старте урока (не только по «Дальше»). Принятые v4 §2б/§3б и `lesson_engine` (состояние `lesson_hook` и переходы) — дословно НЕ тронуты; меняется только транспортный маппинг hook↔контент и расчёт LessonProgress. keeper/контент НЕ тронуты.
...
> **`evt_hook_read` может деривиться СЕРВЕРОМ АВТОМАТИЧЕСКИ на старте урока (E8), а не только по «Дальше» (E9) — РЕВИЗИЯ R3 (Вариант А, hook-маппинг).** В текущем контенте Блока 1 у уроков НЕТ стадии `hook` (см. §3.1, правило hook-маппинга): тогда `fsm_service` на E8 после `evt_start_lesson → lesson_hook` СРАЗУ, в том же шаге, диспатчит `evt_hook_read → lesson_theory`, и клиент в ответе E8 получает уже `lesson_theory` на `theory[0]` (`lesson_hook` — нулевая-контентная пройная-стадия). Отдельного клиентского `advance` для прохода `lesson_hook` в этом случае НЕТ — строка `lesson_hook → evt_hook_read` таблицы выше применяется только когда hook-сообщение В CSV ПРИСУТСТВУЕТ (тогда `lesson_hook` рендерит его, и `evt_hook_read` деривится по «Дальше» как обычно). Правило-якорь: «hook рендерится, только если стадия hook присутствует в CSV; иначе авто-проскок на старте». Это транспортная деривация существующего перехода `lesson_hook --evt_hook_read--> lesson_theory` (v4 §2б), НЕ новое FSM-поведение и НЕ новый переход — симметрично серверной деривации `evt_warmup_complete`/`evt_session_end`. См. §3.1 (правило) и §4.5 (ремап LessonProgress).
...
| `LessonProgress` (этап 1–6/1–5) | сервер (по stage + наличию hook, §4.5/R3) | производное от `fsm_state` |
...
#### 3.1-R3 Правило hook-маппинга: hook рендерится, только если стадия hook присутствует в CSV — РЕВИЗИЯ R3 (Вариант А, hook-маппинг)

**Факт (подтверждён при кодировании п.1a):** ни в одном уроке Блока 1 (все 9 уроков) НЕТ строки со стадией `hook` — уроки начинаются со стадии `theory` (последовательность стадий в текущем контенте: `theory → example → training → main_question → main_question_backup → lesson_failed → final → repeat_1h → repeat_evening → repeat_morning`). При этом `keeper.LESSON_STAGES` НЕ содержит `hook` — CSV со стадией `hook` был бы ОТКЛОНЁН keeper. Но FSM v4 / `lesson_engine` имеют состояние `lesson_hook` (`lesson_select --evt_start_lesson--> lesson_hook --evt_hook_read--> lesson_theory`), куда `fsm_service.start_lesson` приводит ученика на старте — а рендерить там нечего. Нестыковка устраняется транспортным правилом (БЕЗ изменения FSM/движка/keeper):

1. **Если у урока в CSV НЕТ сообщения стадии `hook` (текущая реальность всего контента):** `fsm_service` при старте урока (E8: `lesson_select --evt_start_lesson--> lesson_hook`) **АВТОМАТИЧЕСКИ деривит `evt_hook_read` на том же шаге**, и ученик попадает СРАЗУ в `lesson_theory` на ПЕРВОЕ сообщение стадии `theory` (`theory[0]`). То есть `lesson_hook` становится **нулевой-контентной пройной-стадией**, когда hook-сообщения нет: отдельного клиентского «Дальше» (E9) для прохода `lesson_hook` НЕ требуется. Это по аналогии с серверной деривацией `evt_warmup_complete` (§2.5) / `evt_session_end` (§6.1) — сервер деривит транзитное событие, не дожидаясь клиентского действия.
2. **Если у урока в CSV ПОЯВИТСЯ hook-сообщение (будущее):** `lesson_hook` рендерит его штатно, как любую не-вопросную многоэкранную стадию (§3.1 п.3), и `evt_hook_read` деривится по «Дальше» (E9) как обычно (строка `lesson_hook → evt_hook_read` таблицы §1.3 применяется именно в этом случае).
3. **Правило-якорь:** «hook рендерится, ТОЛЬКО если стадия `hook` присутствует в CSV урока; иначе авто-проскок `lesson_hook → lesson_theory` на старте». Сервер определяет наличие hook по плоскому списку сообщений урока (есть ли строка `stage=hook`) — клиент этого не знает и не присылает.
4. **Вырожденный случай «есть hook, нет theory» НЕВОЗМОЖЕН — отсечён шлюзом keeper ДО рантайма (косметика по ревью Критика).** Стадия `theory` обязательна по `keeper.REQUIRED_STAGES` (структурный контракт keeper.py): урок без сообщения стадии `theory` был бы ОТКЛОНЁН keeper и не дошёл бы до движка/транспорта. Поэтому конфигурация «hook присутствует, а theory отсутствует» физически не достижима в рантайме — пустой `theory` после прохождения `lesson_hook` (как при наличии hook, так и при авто-проскоке) невозможен. **Причина невозможности: keeper-шлюз требует `theory` обязательной стадией.** Кодеру НЕ нужно предусматривать ветку «hook прошли, а theory пустая» — она вырезана валидацией контента до рантайма (CLAUDE.md §3, keeper стережёт структуру). Это пометка «невозможно + причина», а не новое поведение; keeper НЕ трогается (правило лишь опирается на уже действующий `keeper.REQUIRED_STAGES`).

> **Дисциплина R3:** это ТРАНСПОРТНОЕ уточнение маппинга hook↔контент. Принятые v4 §2б/§3б и `lesson_engine` — состояние `lesson_hook` и переход `lesson_hook --evt_hook_read--> lesson_theory` — остаются ДОСЛОВНО, новых состояний/событий/переходов нет. keeper/контент НЕ трогаются (наоборот: правило опирается на факт, что `keeper.LESSON_STAGES` без `hook` и `keeper.REQUIRED_STAGES` с обязательной `theory`). Влияние на расчёт `LessonProgress` — §4.5; кросс-ссылка в §1.3 (авто-деривация `evt_hook_read` на E8); E8 (§4.3) отдаёт первый экран сразу как `lesson_theory`/`theory[0]`.
...
```json
{
  ...
  "lesson_progress": {                // ? присутствует в lesson_* view; индикатор A6 §3.2 «этап». total=6 при наличии hook-стадии, total=5 при её отсутствии (R3, §4.5)
    "step": 3,                        // 1..total: при hook — 1=hook,2=theory,3=example,4=training,5=main_question,6=final; БЕЗ hook (текущий контент Блока 1) — 1=theory,2=example,3=training,4=main_question,5=final (§4.5)
    "total": 6                        // 6 при наличии hook-сообщения в CSV; 5 при отсутствии (R3) — для контента Блока 1 = 5
  },
...
**Response 200 (урок есть, hook-сообщение присутствует в CSV):** `render` с `view: "lesson_message"`, `message` = первое `hook`-сообщение, `lesson_progress.step=1`, `lesson_progress.total=6`, `fsm_state: "lesson_hook"`.
**Response 200 (урок есть, hook-стадии в CSV НЕТ — текущий контент Блока 1, R3 §3.1):** сервер на старте авто-деривит `evt_hook_read → lesson_theory` (без клиентского «Дальше»), `render` отдаётся УЖЕ с `view: "lesson_message"`, `message` = первое `theory`-сообщение (`theory[0]`), `lesson_progress.step=1`, `lesson_progress.total=5`, `fsm_state: "lesson_theory"`. То есть для текущего контента ученик в одном ответе E8 получает `theory=1/5`, минуя пустой `lesson_hook`.
...
**Response 200 (correct → final):** main-correct#1/#2 → `view: "lesson_final"`, `message` = `final`-сообщение, `lesson_progress.step` = последний (= `total`).
...
#### 4.5-R3 Расчёт `LessonProgress` (ремап под hook-маппинг) — РЕВИЗИЯ R3 (Вариант А)

`LessonProgress` (A6 §3.2 «этап») сервер деривит из `fsm_state`. До R3 шкала была жёстко `1..6` с `1=hook`. По Варианту А расчёт зависит от наличия стадии `hook` в CSV урока (правило §3.1-R3):

- **Если hook-сообщение ПРИСУТСТВУЕТ в CSV урока:** `total=6`, нумерация как было — `1=hook, 2=theory, 3=example, 4=training, 5=main_question, 6=final`.
- **Если hook-стадии в CSV НЕТ (текущая реальность Блока 1 — все 9 уроков):** `hook` сливается с `theory`, прогресс считается из **5 шагов, НЕ 6**: `total=5`, нумерация — `1=theory, 2=example, 3=training, 4=main_question, 5=final`. То есть в текущем контенте Блока 1 прогресс — `*/5`, и старт урока — `theory=1/5` (не `hook=1/6`).
- **`step` считается по фактически присутствующим не-вопросным+вопросным стадиям** урока (по `fsm_state`), а `total` = 6 при наличии hook-стадии, 5 при её отсутствии. Источник наличия hook — плоский список сообщений урока (§3.1-R3), сервер вычисляет; клиент не присылает.

**Маппинг `fsm_state → step` (БЕЗ hook, total=5 — текущий контент):**

| `fsm_state` | step / total |
|-------------|--------------|
| `lesson_theory` | 1 / 5 |
| `lesson_example` | 2 / 5 |
| `lesson_training` | 3 / 5 |
| `lesson_main_question` / `lesson_main_question_backup` / `lesson_theory_review` | 4 / 5 |
| `lesson_final` | 5 / 5 |

> **Полнота таблицы для `total=6` (hook ПРИСУТСТВУЕТ) — косметика по ревью Критика, снимает домысливание у кодера.** При `total=6` все строки таблицы выше сдвигаются на **+1**, начиная с нового первого шага `1=hook`: `lesson_hook`→1/6, `lesson_theory`→2/6, `lesson_example`→3/6, `lesson_training`→4/6, `lesson_main_question`/`lesson_main_question_backup`/`lesson_theory_review`→5/6, `lesson_final`→6/6. **`lesson_theory_review` остаётся на ШАГЕ `lesson_main_question`** (4/5 при total=5; 5/6 при total=6) — это тот же методический шаг «главный вопрос», а НЕ отдельная ячейка индикатора (теория-повтор внутри попытки главного вопроса, не новый этап прогресса). **`lesson_failed` и авто-проскок `lesson_hook` (при total=5) отдельной ячейки индикатора НЕ получают:** `lesson_failed` — терминал провала (свой `view: "lesson_failed"`, индикатор этапов не рисуется как продвижение), а `lesson_hook` при отсутствии hook — нулевая-контентная пройная-стадия (§3.1-R3), на которой ученик не задерживается, поэтому в шкале `*/5` шага у неё нет.
> `lesson_hook` в контенте без hook — нулевая-контентная пройная-стадия (§3.1-R3), на ней клиент не задерживается (авто-проскок на E8), поэтому в шкале `*/5` отдельного шага у неё нет. При наличии hook — `total=6` и строки сдвигаются на +1 с `1=hook`. Это ТРАНСПОРТНЫЙ ремап индикатора прогресса; FSM/движок не меняются (§3.1-R3 дисциплина).

> **ВСТРЕЧНАЯ ПРАВКА A6 §3.2 НА ПРИЁМКУ (СУЩЕСТВЕННО, по ревью Критика).** Этот ремап `LessonProgress` (динамический `total`: 5 при отсутствии hook-стадии / 6 при наличии; макет шапки рисует `total` ячеек, а не фиксированные 6) **переопределяет принятый A6 §3.2**, где `LessonProgress` жёстко описан как «этап 1–6» и макет шапки нарисован фиксированным `[▮▮▯▯▯▯]` (6 ячеек). Нельзя оставлять два принятых артефакта противоречащими (CLAUDE.md §3 — рассинхрон принятых артефактов закрывается осознанно, а не молча). **Встречная правка A6 §3.2 на приёмку:** `LessonProgress` — динамический `total` (5 при отсутствии hook-стадии / 6 при наличии); макет шапки рисует `total` ячеек, а не фиксированные 6. Текущий контент Блока 1 → `*/5`. Помечено на сверку с A6/приёмку фаундера (CLAUDE.md §3 — рассинхрон принятых артефактов закрывается осознанно). **A6 сам в этом аддендуме НЕ редактируется** — архитектор только помечает встречную правку; финальное приведение A6 §3.2 к динамическому `total` — на приёмке фаундера/в зоне A6.

### 4.6 E7 `GET /api/lesson/current` — resume (EC-02)

**Request:** без тела (cookie).
**Response 200:** `render` ТЕКУЩЕГО сохранённого сообщения (`Progress.current_message_id`), `fsm_state` из профиля, `view: "lesson_message"`/`lesson_question` (БЕЗ блока `feedback` — resume не несёт результата ответа), `lesson_progress` по правилу §4.5/R3 (total=5 для текущего контента). Если `fsm_state` не в уроке (`registered` с in_progress-прогрессом, после S-10 cancel) — `view: "lesson_message"` с resume-точкой + флаг возобновления; клиент показывает «Продолжить урок». Если контент не подгрузился офлайн (D-5) — клиент сам рендерит `connection_stub` (это клиентский fallback, не отдельный код сервера); сервер при наличии контента всегда отдаёт сообщение.
...
| старт урока, hook ОТСУТСТВУЕТ (R3, текущий контент) | `lesson_message` | нет | `theory[0]`; `fsm_state=lesson_theory`, `lesson_progress=1/5` (авто-проскок hook, §4.3/§4.5) |
| старт урока, hook ПРИСУТСТВУЕТ | `lesson_message` | нет | `hook[0]`; `fsm_state=lesson_hook`, `lesson_progress=1/6` |
...
| `evt_hook_read` | **КЛИЕНТ (action=advance, если hook ПРИСУТСТВУЕТ)** ИЛИ **СЕРВЕР деривит автоматически на старте урока (E8), если hook-стадии в CSV НЕТ (R3, §3.1)** | HTTP | E9 (при наличии hook) / E8 (авто-проскок при отсутствии hook, §3.1-R3) |
| `evt_theory_read`/`evt_example_read`/`evt_theory_reviewed`/`evt_lesson_complete`/`evt_lesson_fail_confirmed` | КЛИЕНТ (action=advance) → сервер деривит evt по stage | HTTP | E9 `POST /api/lesson/advance` |
...
| E8 `/lesson/start` | `evt_start_lesson` / `evt_no_lesson_today` / `evt_all_lessons_done` (по манифесту); **при отсутствии hook — авто `evt_hook_read` на том же шаге (R3, §3.1)** | `lesson_select → lesson_hook`/`daily_done`/`daily_blocked`/`course_complete`; **`lesson_hook → lesson_theory` авто при отсутствии hook** | v4 §2б; §3.1-R3 |
| E9 `/lesson/advance` (по stage) | `evt_hook_read` (**только если hook ПРИСУТСТВУЕТ, R3 §3.1**)/`evt_theory_read`/`evt_example_read`/`evt_theory_reviewed`/`evt_lesson_complete`/`evt_lesson_fail_confirmed` | соответствующие lesson-переходы | v4 §2б; §1.3 |
...
> Все события `evt_*` и их guards — дословно как в v4 §2б / `lesson_engine.py`. Аддендум их не меняет; он лишь говорит, КАКОЙ HTTP-вызов их инициирует и КАК сервер деривит событие из действия+stage (анти-подмена). После R1 карта покрывает ВСЕ `evt_*` ученика: HTTP (E5–E12), сервер-деривит (`evt_warmup_complete`, `evt_session_end`, **`evt_hook_read` при отсутствии hook — R3**), scheduler (4 временных), сервер-внутренне (`evt_streak_updated`), вне охвата (регистрация/auth — с явной причиной). Тупиков HTTP-возврата нет.

---

## 9. Самопроверка перед выдачей

- [x] Не переоткрыты FSM/матрица прав/сценарии/edge/failure v4 — только HTTP-биндинг. FSM-YAML и permissions-YAML НЕ дублированы (намеренно, §0); validator.py по этому документу неприменим — ожидаемо, не дефект. **R1: несущие контракты v4 §2б/§3б и `lesson_engine.py` (события/гарды/side-effects/матрица прав/инвариант атомарности) НЕ тронуты ни одной из 7 правок. R2: пять правок R2 (№1–№5) — тоже чисто транспортные/контракт-данных-на-приёмку; FSM-YAML не введён, несущие контракты v4 НЕ тронуты. R3 (Вариант А, hook-маппинг): три точечные транспортные правки (§3.1-R3 правило hook-маппинга, §4.5-R3 ремап LessonProgress 5/6, §1.3 кросс-ссылка авто-деривации evt_hook_read) — состояние `lesson_hook` и переход `lesson_hook --evt_hook_read--> lesson_theory` v4 §2б/`lesson_engine` НЕ тронуты дословно; keeper/контент НЕ тронуты; формализуется без validator (чисто транспортное уточнение, решение фаундера). R3 ДОУТОЧНЕНА по ревью Критика (3 косметико-существенные правки, блокеров нет): пометка встречной правки A6 §3.2 на приёмку (§4.5-R3 + ниже), полнота таблицы `fsm_state→step` при `total=6` (§4.5-R3), ссылка на `keeper.REQUIRED_STAGES` для отсечения мнимой дыры «hook без theory» (§3.1-R3 п.4) — A6/v4/движок/keeper по-прежнему НЕ редактировались.**
- [x] **(п.1)** Набор endpoints E4–E12 под дневной поток + урок + R1/R2; ссылочный маппинг «HTTP→evt» (§1.3, §8). Единая кнопка «Дальше» (E9) → разные evt по stage — сервер деривит по `fsm_state`, клиент шлёт только `action:advance` (§1.3). **R3:** `evt_hook_read` может деривиться сервером авто на старте (E8) при отсутствии hook-стадии — кросс-ссылка в §1.3 + §6/§8.
...
- [x] **(п.3)** Схемы request/response + общий render-payload (§4). **R1-№6:** «wrong с возвратом» сведён к единому `view: "lesson_feedback"` (feedback+message в одном payload; различие training/main-wrong#1 — по `fsm_state`); §4.5/§4.9 согласованы. **R1-№7:** `streak_days` помечен как ОПЦИОНАЛЬНЫЙ тихий показ (F-1, A8 `day.counter.badge`); сервер не предписывает заметность. **R2-№5:** зафиксирован дефолт клиента ДО решения по F-1 — тихий/скрытый показ (fail-safe в сторону Methodology §1.3 «без давления»), а не заметный счётчик (§4.1, ремарка про `streak_days`). **R3:** `LessonProgress` ремапнут (§4.5-R3, §4.1): `total=6` при наличии hook, `total=5` при отсутствии (текущий контент Блока 1 — `*/5`, старт `theory=1/5`); E8 (§4.3) при отсутствии hook отдаёт первый экран сразу как `lesson_theory`/`theory[0]`; §4.9 дополнена строками старта урока с/без hook; E7 (§4.6) считает прогресс по тому же правилу. **R3-доуточнение (ревью Критика):** в §4.5-R3 добавлена полнота таблицы `fsm_state→step` для `total=6` (сдвиг +1 с `1=hook`; `lesson_theory_review` остаётся на шаге `main_question`; `lesson_failed`/авто-проскок `lesson_hook` отдельной ячейки не получают) — снимает домысливание у кодера.
- [x] **(п.4а)** Секвенирование внутри урока (§3.1); `training_remaining` и автопереход `training→main_question` (§3.2). **R3:** правило hook-маппинга (§3.1-R3) — hook рендерится только при наличии стадии в CSV, иначе авто-проскок `lesson_hook → lesson_theory` на старте; опирается на факт `keeper.LESSON_STAGES` без `hook` и `theory`-первая-стадия во всех 9 уроках Блока 1. **R3-доуточнение (ревью Критика):** §3.1-R3 п.4 — вырожденный случай «есть hook, нет theory» НЕВОЗМОЖЕН, отсечён `keeper.REQUIRED_STAGES` (стадия `theory` обязательна по keeper → пустой `theory` после hook невозможен до рантайма); пометка «невозможно + причина», keeper НЕ трогается. **(п.4б)** Манифест курса (§3.3); `COURSE_MANIFEST` в config.py — правка контракта данных на приёмку (§3.4); отвергнут вывод из имён файлов. Не ORM-миграция. **R2-№1:** §3.4-bis — требование ЕДИНОГО пространства `lesson_id` для COURSE_MANIFEST / Progress.lesson_id / ключа контента csv_loader; зафиксирован рассинхрон (csv_loader ключует по имени файла; col3 `lesson_id="1"` на весь блок; 9 файлов вместо 27); предложен интерфейс (I) ключевание load_lessons_dir по col3 ИЛИ (II) `lesson_messages(lesson_id)`; помечено ЧЕСТНО как КОНТЕНТНЫЙ блокер (keeper.py/контент-продюсер + приёмка фаундера), НЕ код; конкретные 27 id НЕ выдуманы.
...
- [x] **Правки на приёмку фаундеру/Brain-дельту:** (1) `COURSE_MANIFEST` в config.py — добавление конфиг-константы (§3.4); (1-bis, R2-№1) единое пространство `lesson_id` для COURSE_MANIFEST/Progress/контента + выбор интерфейса (I/II) — правка §3-контракта данных; приведение контента к уникальному `lesson_id` и к 27 урокам — зона keeper.py/контента, КОНТЕНТНЫЙ блокер (§3.4-bis); (2) опционально `Progress.row_version` — только если приёмка требует строгий optimistic lock (§5.4, по умолчанию не применяется). §3-контракт данных, помечены явно; models.py commit ae6e765 в базовом варианте НЕ меняется. **(3) ВСТРЕЧНАЯ ПРАВКА A6 §3.2 на приёмку (R3-доуточнение, ревью Критика):** ремап `LessonProgress` (динамический `total` — 5 при отсутствии hook-стадии / 6 при наличии; макет шапки рисует `total` ячеек, а не фиксированные 6; текущий контент Блока 1 → `*/5`) переопределяет принятый A6 §3.2 («этап 1–6», макет `[▮▮▯▯▯▯]` = 6 ячеек). Два принятых артефакта не должны противоречить (CLAUDE.md §3 — рассинхрон закрывается осознанно). Помечено на сверку с A6/приёмку фаундера (§4.5-R3); A6 в этом аддендуме НЕ редактируется — только встречная пометка. **R3 (hook-маппинг) — НЕ правка контракта данных и НЕ продуктовая развилка:** чисто транспортное уточнение маппинга hook↔контент + расчёта LessonProgress; решение фаундера (Вариант А); формализуется без validator; keeper/контент/FSM/движок не тронуты.
- [x] Нерешённых ПРОДУКТОВЫХ развилок аддендум не создаёт; §3.6 (рандомизация backup-формулировки, календарное расписание), §6.1-альтернатива (lazy-on-E4 vs nightly-job для `evt_session_end`; `daily_blocked` исключён в обоих, R2-№3), §2.5-skip-трактовка — НЕ блокеры, дефолты безопасны и не противоречат Методологии; помечены для сверки А4. F-1 (заметность streak) — продуктовая развилка фаундера; аддендум лишь даёт fail-safe-дефолт «тихо» (R2-№5), не закрывает её. **R3 (hook-маппинг) — НЕ продуктовая развилка** (решение фаундера принято, Вариант А): формализован транспортно, без отсебятины. Встречная правка A6 §3.2 (динамический `total`) — НЕ продуктовая, это сверка двух принятых артефактов на согласованность, помечена на приёмку.

---

*Аддендум v1 (ревизии R1 + R2 + R3, 2026-06-22/23) — транспортный HTTP-биндинг дневного потока и прохождения урока (R1/R2 включены) поверх принятой `specs/student_lesson_fsm_v4.md` (validator PASS, А4 GO) и `lesson_engine.py`. Стыкуется с `specs/student_registration_api_v1.md` через `next:daily_start`. Не меняет события evt_*, guards, side-effects, deny-by-default матрицы прав v4, инвариант «fsm_state — канонический источник, обновляется атомарно с Progress». Серверное судейство ответа (анти-подмена, §2), порядок «гард до инкремента» (§2.2-bis), R3-разминка + skip-не-сжигает-due (§2.5), секвенирование внутри урока + hook-маппинг (§3.1/§3.1-R3), момент автоперехода (§3.2), манифест курса (§3.3) + единое пространство lesson_id (§3.4-bis), ремап LessonProgress 5/6 под hook-маппинг (§4.5-R3), дедуп переходов через sequence-эхо без новой колонки — включая ветвь E9 по позиционному курсору и `seq++` на каждый command (§5.2), конвенция кодов и маппинг исключений движка (§5), граница scheduler + `evt_session_end` (`daily_blocked` исключён, §6/§6.1) — транспортно-архитектурные решения, обоснованы. РЕВИЗИЯ R3 (Вариант А, hook-маппинг, решение фаундера, без validator): hook рендерится только при наличии стадии в CSV, иначе авто-проскок lesson_hook→lesson_theory на старте + LessonProgress */5; FSM/движок/keeper НЕ тронуты. R3 доуточнена по ревью Критика (GO WITH REVISIONS, блокеров нет): встречная правка A6 §3.2 на приёмку (динамический total), полнота таблицы fsm_state→step при total=6, отсечение мнимой дыры «hook без theory» через keeper.REQUIRED_STAGES — A6/v4/движок/keeper НЕ редактировались. ПРАВКИ КОНТРАКТА ДАННЫХ НА ПРИЁМКУ: COURSE_MANIFEST в config.py (§3.4, обязательна для guard'ов lesson_select) + единое пространство lesson_id / интерфейс csv_loader (§3.4-bis, R2-№1, включает КОНТЕНТНЫЙ блокер на стороне keeper.py/контента); Progress.row_version (§5.4, опциональна, по умолчанию НЕ применяется). ВСТРЕЧНАЯ ПРАВКА A6 §3.2 (динамический total LessonProgress) — на сверку/приёмку, A6 не редактировался. Проверяют: А4 + validator.py.*
