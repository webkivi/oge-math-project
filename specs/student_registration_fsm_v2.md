...
| D-1 | Смена имени/класса после регистрации — **наследие v4 §0, живёт там.** | Остаётся открытой в v4; здесь — кросс-ссылка, новых обязательств не несёт. Регистрация в этой спеке только СОЗДАЁТ профиль; `write` по профилю — deny (как в v4). | Не относится к onboarding напрямую. |
