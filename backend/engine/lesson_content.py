...
валидации (битый урок не блокирует индекс), а keeper прогоняется только при
фактической загрузке конкретного урока (`load_lesson`). Запрос урока, не
прошедшего keeper → InvalidCSVError — это штатный fallback EC-08 / F-03, который
обрабатывает вызывающий слой (на 2026-06-28 все 9 уроков Блока 1 проходят keeper,
этот fallback покрывает будущий легаси/битый контент).
"""
